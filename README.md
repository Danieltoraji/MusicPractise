...
- `#/` 首页（系列与独立关卡） · `#/series/:id` 系列详情（专题→关卡+进度） · `#/library` 资源库（导入/导出/删除/试玩） · `#/level/:id` 运行关卡 · `#/graph/:id` 逻辑图谱（只读节点图）
- 内置五关 + 听音点击·测试 + 入门系列自动入库；支持导入 `.json` 关卡与 `.zip` 系列包、导出自包含 zip；编辑器「新建关卡 / 编辑副本」
- 练习进度（最高分/通过/次数）存 IndexedDB，首页与系列页展示
